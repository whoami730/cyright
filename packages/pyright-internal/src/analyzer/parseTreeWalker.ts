...
export function getChildNodes(node: ParseNode): (ParseNode | undefined)[] {
    switch (node.nodeType) {
        ...
        case ParseNodeType.For:
            // ! Cython
            return [node.targetExpression, node.iterableExpression, node.byExpression, node.forSuite, node.elseSuite];
        ...
        case ParseNodeType.TypeParameter:
            // ! Cython
            return [node.name, node.boundExpression, node.defaultExpression, node.defaultValue];
        ...
        case ParseNodeType.CType:
            return [node.expression, node.typeTrailNode, node.varTrailNode];

        case ParseNodeType.CVarTrail:
            return node.nodes;

        case ParseNodeType.CTypeTrail:
            return [...node.argumentLists.flat(), node.postMemberNode];
        ...
        case ParseNodeType.CCast:
            return [node.typeNode, node.valueExpression];
        ...
        case ParseNodeType.CSizeOf:
            return [node.leftExpression, node.valueExpression];
...
export class ParseTreeVisitor<T> {
    ...
    visit(node: ParseNode): T {
        switch (node.nodeType) {
            ...
            // ! Cython
            case ParseNodeType.CTypeDef:
                return this.visitCTypeDef(node);

            case ParseNodeType.CType:
                return this.visitCType(node);

            case ParseNodeType.CVarTrail:
                return this.visitCVarTrail(node);

            case ParseNodeType.CTypeTrail:
                return this.visitCTypeTrail(node);

            case ParseNodeType.CExtern:
                return this.visitCExtern(node);

            case ParseNodeType.CTupleType:
                return this.visitCTupleType(node);

            case ParseNodeType.CCallback:
                return this.visitCCallback(node);

            case ParseNodeType.CAddressOf:
                return this.visitCAddressOf(node);

            case ParseNodeType.CCast:
                return this.visitCCast(node);
            ...
            case ParseNodeType.CDefine:
                return this.visitCDefine(node);

            case ParseNodeType.CSizeOf:
                return this.visitCSizeOf(node);

            case ParseNodeType.CBlockTrail:
                return this.visitCBlockTrail(node);

            case ParseNodeType.CGil:
                return this.visitCGil(node);

            case ParseNodeType.CNew:
                return this.visitCNew(node);
